#include < stdio.h > #include < stdlib.h > #include < string.h > #include < unistd.h > #include < sys / types.h > #include < sys / socket.h > #include < netinet / in .h > #include < arpa / inet.h > #include "Practical.h"

int main(int argc, char * argv[]) {
    char recvbuffer[BUFSIZE], sendbuffer[BUFSIZE], request_resp[20], balance_msg[20]; // text buffers 13
    ...
    in_port_t servPort = atoi(argv[2]); // Create a reliable, stream socket using TCP 25
    int sock = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    if (sock < 0)
        DieWithSystemMessage("socket() failed"); // Construct the server address structure
    struct sockaddr_in servAddr; // Server address
    memset( & servAddr, 0, sizeof(servAddr)); // Zero out structure
    servAddr.sin_family = AF_INET; // IPv4 address family // Convert IP address
    ...
    servAddr.sin_port = htons(servPort); // Server port // Establish the connection to the movieServer
    ...
    sscanf(recvbuffer, "%s\r\n%s %d", request_resp, balance_msg, & balance); // message expected is of the form
    //MOVIE_GRANTED\r\nCURRENT_BALANCE: %d\r\n\r\n
    //or, MOVIE_REJECTED\r\nCURRENT_BALANCE: %d\r\n\r\n
    if (strcmp(request_resp, "MOVIE_GRANTED") == 0) {
		...
		fputc('\n', stdout); // Print a final linefeed 95
